  ...
  const fetchReports = async () => {
    try {
      ...
      const customersResponse = await axios.get(
        "http://localhost:5001/api/reports/top-customers",
        {
      ...
      const performanceResponse = await axios.get(
        "http://localhost:5001/api/reports/performance"
      );
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5001/api/reports/export",
        {
          params: filters,
          responseType: "blob",
        }
      ...
      <div className="stats-grid">
          ...
          {performance && (
            <div
              className={`stat-change ${
                performance.growth.salesGrowth >= 0 ? "positive" : "negative"
              }`}
            >
              {performance.growth.salesGrowth >= 0 ? "↑" : "↓"}{" "}
              {Math.abs(performance.growth.salesGrowth)}%
            </div>
          )}
        </div>
        ...
        <div className="stat-card">
          ...
          {performance && (
            <div
              className={`stat-change ${
                performance.growth.revenueGrowth >= 0 ? "positive" : "negative"
              }`}
            >
              {performance.growth.revenueGrowth >= 0 ? "↑" : "↓"}{" "}
              {Math.abs(performance.growth.revenueGrowth)}%
            </div>
          )}
        </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                labelLine={false}
                label={(entry) =>
                  `${entry._id.name}: $${entry.totalRevenue.toFixed(0)}`
                }
                outerRadius={80}
                fill="#8884d8"
                dataKey="totalRevenue"
